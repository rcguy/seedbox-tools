...
def load_config(script_cwd: str, cli_opts: argparse.Namespace) -> SeedboxConfig:
    ...
    if args.client == "deluge" and args.filter:
        filter_dict = dict(map(lambda s: s.split('='), args.filter))
    else:
        filter_dict = {}
    
    # Determine export dir
    export_dir = os.path.abspath(args.export_dir) if args.export_dir else os.path.join(script_cwd, cfg["export_dir"], args.client)
